...
unsigned int enc28j60PhyRead(unsigned char address)
{
    ...
    unsigned char h = enc28j60Read(MIWRH);
    ret = (enc28j60Read(MIWRL)) | (h << 8);
    enc28j60Write(MICMD, 0); /*Stop reading*/
    return ret;
}
...
{   
    ...
    __delay_ms(200);
    unsigned char revid = enc28j60Read(EREVID);
    revid = enc28j60Read(ECOCON);
    revid = enc28j60Read(EFLOCON);
